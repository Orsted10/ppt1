...
const Slide8: React.FC<SlideProps> = ({ isActive, currentStep, onTotalStepsChange }) => {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      if (titleRef.current) scrambleText(titleRef.current, "CLINICAL TRIAL\nOPTIMIZATION", 1200);
  ...
  useEffect(() => {
    ...
    if (currentStep === 1) {
      gsap.to(chartLinesRef.current, {
        scaleY: (i) => 1 - (i * 0.15), opacity: 1, duration: 1, stagger: 0.1, ease: 'power4.out'
      });
    }
    ...
    <div ref={containerRef} className="slide-container" style={{ zIndex: isActive ? 10 : 1 }}>
      <div className="slide-content" style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <div style={{ flex: 1, paddingRight: '4rem' }}>
          ...
          <div className="tech-panel interactable" style={{ marginTop: '2rem' }}>
        ...
        <div className="interactable" style={{ flex: 1, height: '400px', display: 'flex', alignItems: 'flex-end', gap: '20px', borderBottom: '2px solid var(--border)', paddingBottom: '1rem' }}>
          {[1, 2, 3, 4, 5].map((_, i) => (
            <div
              key={i}
