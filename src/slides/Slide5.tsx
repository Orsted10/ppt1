...
const Slide5: React.FC<SlideProps> = ({ isActive, currentStep, onTotalStepsChange }) => {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      gsap.set([hex1Ref.current, hex2Ref.current, hex3Ref.current, hex4Ref.current], { scale: 0, opacity: 0 });
  ...
  useEffect(() => {
    ...
    if (currentStep === 1) gsap.to(hex1Ref.current, { scale: 1, opacity: 1, duration: 0.5, ease: 'expo.out' });
    if (currentStep === 2) {
      gsap.to(linesRef.current[0], { scaleX: 1, duration: 0.3, transformOrigin: 'left' });
      gsap.to(hex2Ref.current, { scale: 1, opacity: 1, duration: 0.5, ease: 'expo.out', delay: 0.2 });
    }
    if (currentStep === 3) {
      gsap.to(linesRef.current[1], { scaleX: 1, duration: 0.3, transformOrigin: 'left' });
      gsap.to(hex3Ref.current, { scale: 1, opacity: 1, duration: 0.5, ease: 'expo.out', delay: 0.2 });
    }
    if (currentStep === 4) {
      gsap.to(linesRef.current[2], { scaleX: 1, duration: 0.3, transformOrigin: 'left' });
      gsap.to(hex4Ref.current, { scale: 1, opacity: 1, duration: 0.5, ease: 'expo.out', delay: 0.2 });
    ...
    if (currentStep < 4) {
      gsap.to(hex4Ref.current, { scale: 0, opacity: 0, duration: 0.3 });
    ...
    if (currentStep < 3) {
      gsap.to(hex3Ref.current, { scale: 0, opacity: 0, duration: 0.3 });
      gsap.to(linesRef.current[1], { scaleX: 0, duration: 0.3 });
    }
    if (currentStep < 2) {
      gsap.to(hex2Ref.current, { scale: 0, opacity: 0, duration: 0.3 });
      gsap.to(linesRef.current[0], { scaleX: 0, duration: 0.3 });
    }
    if (currentStep < 1) gsap.to(hex1Ref.current, { scale: 0, opacity: 0, duration: 0.3 });
